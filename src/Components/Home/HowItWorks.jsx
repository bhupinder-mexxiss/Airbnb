  ...
  return (
    <div className="py-16 bg-[#F7F6FF]">
      <div className="container mx-auto">
        ...
        <div className="mt-12">
          <div className="grid grid-cols-2 gap-x-6 sm:gap-x-0 sm:flex flex-wrap items-center justify-between gap-y-10 md:gap-y-16">
            {data.map((item, index) => (
              <div className="md:w-1/3 xl:w-1/4 md:even:mt-14">
                <div className="flex flex-col items-center justify-center gap-3 max-w-[200px] w-full h-[180px] sm:h-[200px] rounded-full rounded-tr-none shadow-xl bg-white mx-auto p-3 relative">
                  <span className="absolute right-2 top-2 text-2xl sm:text-3xl font-medium text-primary opacity-20">
                    0{index + 1}
                  </span>
                  <img src={item.img} className="w-12 sm:w-[60px]" />
                  <p className="text-center sm:text-lg font-medium text-text1">
                    {item.name}
                  </p>
                </div>
              </div>
